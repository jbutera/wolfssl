                       ...
                       word32 helloSz, byte* extMsgType)
{
    ...
    if ((args->idx - args->begin) < helloSz) {
        ...
        if ((ret = TLSX_ParseVersion(ssl, (byte*)input + args->idx, 
    ...
    *extMsgType = args->extMsgType;
    ...
    if (args->totalExtSz > 0) {
        ...
        ret = TLSX_Parse(ssl, (byte*)input + args->idx, args->totalExtSz, 
            *extMsgType, NULL);
        if (ret != 0) {
        #ifdef WOLFSSL_ASYNC_CRYPT
                       ...
                       word32 helloSz)
{
    ...
    if ((args->idx - args->begin) + OPAQUE16_LEN + RAN_LEN + OPAQUE8_LEN 
                                                                > helloSz) {
    ...
    /* Parse extensions */
    if ((ret = TLSX_Parse(ssl, (byte*)input + args->idx, totalExtSz, client_hello,
                                                            args->clSuites))) {
        goto exit_dch;
    }
    ...
    if (TLSX_Find(ssl->extensions, TLSX_PRE_SHARED_KEY) != NULL) {
        ...
        RefineSuites(ssl, args->clSuites);

        /* Process the Pre-Shared Key extension if present. */
        ret = DoPreSharedKeys(ssl, input + args->begin, helloSz,
            &args->usingPSK);
        if (ret != 0)
            goto exit_dch;
    }
